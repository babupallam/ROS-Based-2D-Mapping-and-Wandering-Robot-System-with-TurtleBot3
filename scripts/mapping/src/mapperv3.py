...
class Map(object):
    ...
    #def __init__(self, origin_x=-2.5, origin_y=-2.5, resolution=.1, width=50, height=50):
    def __init__(self, origin_x=-5, origin_y=-5, resolution=.05, width=100, height=100):
    ...
    def expand_map(self, extra_width=50, extra_height=50):
        """Dynamically expand the map dimensions."""
        ...
        # Calculate offsets for the current grid in the new grid
        offset_x = extra_width // 2
        offset_y = extra_height // 2

        # Copy the old grid into the new grid at the center
        ...
        rospy.loginfo(f"Map expanded: New dimensions - Width: {new_width}, Height: {new_height}")
...
class Mapper(object):
    ...
    def scan_callback(self, scan):
        ...
        for angle_index in range(len(scan.ranges)):
            ...
            # Check if the global coordinates are outside the current grid bounds
