...
class Map(object):
    ...
    def set_cell(self, x, y, val):
        ...
        Arguments:
            x, y  - Coordinates in the map coordinate frame (meters).
            val   - Value to assign to the grid cell (e.g., probability of occupancy).

        Converts the coordinates into grid indices and updates the grid if within bounds.
        """
        ...
        if 0 <= i < self.width and 0 <= j < self.height:
            self.grid[j, i] = val  # Update the grid
        else:
            rospy.logwarn(f"Coordinates ({x}, {y}) are out of map bounds.")
...
class Mapper(object):
    ...
    def scan_callback(self, scan):
        ...
        for angle_index in range(len(scan.ranges)):
            ...
            # Calculate angle and local coordinates
            angle = scan.angle_min + angle_index * scan.angle_increment
            local_x = distance * math.cos(angle)
            local_y = distance * math.sin(angle)

            # Use the refined set_cell() method to update the grid
            self._map.set_cell(local_x, local_y, 1)  # Mark as occupied
